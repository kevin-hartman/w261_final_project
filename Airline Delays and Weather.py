...
# MAGIC %md Review columns with missing data
...
# MAGIC * Cleaning categorical and numerical data
...
flights_and_weather_pipeline_processed.count()
...
airlineGraph = {'nodes': flights_and_weather_combined_processed.select('ORIGIN', 'DEST').rdd.flatMap(list).distinct().collect(), 
                'edges': flights_and_weather_combined_processed.select('ORIGIN', 'DEST').rdd.map(tuple).collect()}
...
flights_and_weather_combined_processed.createOrReplaceTempView("flights_and_weather_combined_processed")
# Now we want to separate the pagerank for the stations based on destination and origin
flights_and_weather_combined_processed = spark.sql("SELECT * from flights_and_weather_combined_processed LEFT JOIN pandasPageRank ON flights_and_weather_combined_processed.ORIGIN == pandasPageRank.Station").drop('Station')
flights_and_weather_combined_processed = flights_and_weather_combined_processed.withColumnRenamed('PageRank', 'ORIGIN_PAGERANK')
flights_and_weather_combined_processed.createOrReplaceTempView("flights_and_weather_combined_processed")
# Repeat for Dest
flights_and_weather_combined_processed = spark.sql("SELECT * from flights_and_weather_combined_processed LEFT JOIN pandasPageRank ON flights_and_weather_combined_processed.DEST == pandasPageRank.Station").drop('Station')
flights_and_weather_combined_processed = flights_and_weather_combined_processed.withColumnRenamed('PageRank', 'DEST_PAGERANK')
flights_and_weather_combined_processed.createOrReplaceTempView('flights_and_weather_combined_processed')

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM flights_and_weather_combined_processed
...
# MAGIC %md # Remaining Analysis on Fully Joined Table

# COMMAND ----------

flights_and_weather_baseline_loc = f"/airline_delays/{username}/DLRS/flights_and_weather_baseline/"
dbutils.fs.rm(flights_and_weather_baseline_loc + 'processed', recurse=True)
...
cleaned_data.write.option('mergeSchema', True).mode('overwrite').format('delta').save(flights_and_weather_baseline_loc + 'processed')

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC DROP TABLE IF EXISTS flights_and_weather_baseline_processed;
# MAGIC 
# MAGIC CREATE TABLE flights_and_weather_baseline_processed
# MAGIC USING DELTA
# MAGIC LOCATION "/airline_delays/$username/DLRS/flights_and_weather_baseline/processed"

# COMMAND ----------

cleaned_data_with_missing_values_corrected = correct_missing_codes(spark.sql("SELECT * FROM flights_and_weather_baseline_processed"))
...
lr_train_preds = model.transform(transformed_train_data)
endTime = time.time()
...
lr_train_metrics = evaluation_metrics(lr_train_preds, "Logistic Regression on training data")
display(lr_train_metrics)

# COMMAND ----------

# MAGIC %md #### Display validation results

# COMMAND ----------

lr_validation_preds = lr_model.transform(transformed_validation_data)
lr_validation_metrics = evaluation_metrics(lr_validation_preds, "Logistic Regression on validation data")
display(lr_validation_metrics)
...
rf_train_preds = rf_model.transform(transformed_train_data)
endTime = time.time()
...
# MAGIC %md #### Display Training Data

# COMMAND ----------

rf_train_metrics = evaluation_metrics(rf_train_preds, "Random Forest on training data")
display(rf_train_metrics)

# COMMAND ----------

# MAGIC %md #### Display Validation Data

# COMMAND ----------

rf_validation_preds = rf_model.transform(transformed_validation_data)
rf_validation_metrics = evaluation_metrics(rf_validation_preds, "Random Forest on validation data")
display(rf_validation_metrics)
